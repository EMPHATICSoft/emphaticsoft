...
Lcount::Lcount(std::vector<std::string> inTags) : tags(inTags) {
...
// Increment operation
void Lcount::click (std::string tag, double amount) {
...
// Readout operations
int Lcount::total(std::string tag) {
    return nint(dTotal(tag));
    
}
double Lcount::dTotal(std::string tag) {
    for (addr i=0; i<tags.size(); ++i) {
        if (tag.compare(tags[i])==0) {
            return count[i];
        }
