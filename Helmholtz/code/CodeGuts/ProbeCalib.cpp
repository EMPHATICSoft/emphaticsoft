...
double ProbeCalib::correctBx(double inBx, bool stdForm) {
    ...
    if (stdForm) return inBx;       // standard form has calibration in it
    int iPoint;
...
double ProbeCalib::correctBy(double inBy, bool stdForm) {
    if (stdForm) return inBy;       // standard form has calibration in it
    int iPoint;
    for (iPoint = 1; iPoint <= nPoints; ++iPoint) {
...
double ProbeCalib::correctBz(double inBz, bool stdForm) {
    if (stdForm) return inBz;       // standard form has calibration in it
    int iPoint;
    for (iPoint = 1; iPoint <= nPoints; ++iPoint) {
