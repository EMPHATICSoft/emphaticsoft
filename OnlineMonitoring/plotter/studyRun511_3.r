...
#
# executable code It assume that the file generated by OnMonT0toRPC are created in 
#
dirDataPath <- Sys.getenv("CETPKG_BUILD"); 
#
mDW9_428_0 <- read.table(sprintf("%s%s", dirDataPath,"/TriggerWaveForms/TrigADC0_Spill9_evt428.txt"), h=T);
mDW9_428_1 <- read.table(sprintf("%s%s", dirDataPath,"/TriggerWaveForms/TrigADC1_Spill9_evt428.txt"), h=T);
...
mD <- read.table(sprintf("%s%s", dirDataPath,"/SummaryTrigT0RPC_V1_511_V1d_60GeV.txt"), h=T); # 
mDTrig <- read.table(sprintf("%s%s", dirDataPath,"/TrigTuple_V1_511_V1d_60GeV.txt"), h=T); # 
mDW10_92_0 <- read.table(sprintf("%s%s", dirDataPath,"/TriggerWaveForms/TrigADCMp0_Spill10_evt92.txt"), h=T);
mDW10_92_1 <- read.table(sprintf("%s%s", dirDataPath,"/TriggerWaveForms/TrigADCMp1_Spill10_evt92.txt"), h=T);
...
#
mDW10_158_0 <- read.table(sprintf("%s%s", dirDataPath,"/TriggerWaveForms/TrigADCMp0_Spill10_evt158.txt"), h=T);
mDW10_158_1 <- read.table(sprintf("%s%s", dirDataPath,"/TriggerWaveForms/TrigADCMp1_Spill10_evt158.txt"), h=T);
...
#
mDW7_1876_0 <- read.table(sprintf("%s%s", dirDataPath,"/TriggerWaveForms/TrigADC0_Spill7_evt1876.txt"), h=T);
mDW7_1876_1 <- read.table(sprintf("%s%s", dirDataPath,"/TriggerWaveForms/TrigADC1_Spill7_evt1876.txt"), h=T);
...
#
mD <- read.table(sprintf("%s%s", dirDataPath,"/SummaryTrigT0RPC_V1_511_V1e_60GeV.txt"), h=T); # 
mDTrig <- read.table(sprintf("%s%s", dirDataPath,"/TrigTuple_V1_511_V1e_60GeV.txt"), h=T); # 
...
ieCut <- 60;
if (ieCut == 4) { 
  mD <- read.table(sprintf("%s%s", dirDataPath,"/SummaryTrigT0RPC_V1_549_V1d_4GeV.txt"), h=T); # 
#
  mDOK <- subset(mD, ((mD$T0OK == 1) & (mD$RPCOK == 1)));
  mDTrig <- read.table(sprintf("%s%s", dirDataPath,"/TrigTuple_V1_549_V1e_4GeV.txt"), h=T); # 
}
#
# Repeat at 8 GeV 
#
if (ieCut == 8) { 
  mD <- read.table(sprintf("%s%s", dirDataPath,"/SummaryTrigT0RPC_V1_536_V1d_8GeV.txt"), h=T); # 
#
  mDOK <- subset(mD, ((mD$T0OK == 1) & (mD$RPCOK == 1)));
  mDTrig <- read.table(sprintf("%s%s", dirDataPath,"/TrigTuple_V1_536_V1e_8GeV.txt"), h=T); # 
}
#
# Repeat at 120 GeV 
#
if (ieCut == 120) { 
  mD <- read.table(sprintf("%s%s", dirDataPath,"/SummaryTrigT0RPC_V1_508_V1e_120GeV.txt"), h=T); # 
#
  mDOK <- subset(mD, ((mD$T0OK == 1) & (mD$RPCOK == 1)));
  mDTrig <- read.table(sprintf("%s%s", dirDataPath,"/TrigTuple_V1_508_V1e_120GeV.txt"), h=T); # 
}
...
hhSpillProfLowTrig <- hist(mDLowTrig$evt, seq(from=0., to=max(mDTrig$evt) + 100, by=20), plot=F);
hhSpillProfHighTrig <- hist(mDHighTrig$evt, seq(from=0., to=max(mDTrig$evt) + 100, by=20), plot=F);
plot(hhSpillProfHighTrig$mids, hhSpillProfHighTrig$counts, type='h', col="grey", xlab=" Event number within a spill", ylab="Counts per 20");
lines(hhSpillProfLowTrig$mids, hhSpillProfLowTrig$counts, type='h', col="red", lwd=2)
...
# T0 analysis.. 
#
mDWT0508_10_4_4 <- read.table(sprintf("%s%s", dirDataPath,"/T0WaveForms/T0ADCBipolar_4_Run_508_Spill10_evt_4.txt"), h=T);
mDWT0508_10_4_5 <- read.table(sprintf("%s%s", dirDataPath,"/T0WaveForms/T0ADCBipolar_5_Run_508_Spill10_evt_4.txt"), h=T);
...
#TDC 
#
mDOKHighTrigT0Adc5T0adc13OKTDCOK <- subset(mDOKHighTrigT0Adc5T0adc13OK, 
                                       ((abs(mDOKHighTrigT0Adc5T0adc13OK$T0tdc5 - 4.8) < 1.) & (abs(mDOKHighTrigT0Adc5T0adc13OK$T0tdc13 - 4.8) < 1.)));
ggplot(mDOKHighTrigT0Adc5T0adc13OKTDCOK, aes(x=T0tdc5, y=T0tdc13)) + geom_point(aes(color=T0adc5-10000), size=0.5) + scale_fill_distiller() + xlim(4.6, 5.1) + ylim(4.6, 5.1);
...
#
#mDOKHighTrigT0Adc5OKTDCOKSmall <- subset(mDOKHighTrigT0Adc5OK, (mDOKHighTrigT0Adc5OK$T0tdc5 < 10000.))
