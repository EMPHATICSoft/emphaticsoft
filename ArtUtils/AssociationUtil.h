...
namespace util {
  ...
  template<class T, class U> static bool CreateAssn(art::Event            & evt,
                                                    std::vector<T>        & a,
                                                    art::Ptr<U>             b,
                                                    art::Assns<T,U>       & assn,
...
//----------------------------------------------------------------------
template<class T, class U> inline bool util::CreateAssn(art::Event           & evt,
                                                        std::vector<T>       & a,
                                                        art::Ptr<U>            b,
                                                        art::Assns<T,U>      & assn,
                                                        size_t                 indx,
                                                        std::string     const& instance)
{
  ...
  try{
    assn.addSingle(art::PtrMaker<T>{evt, instance}(indx), b);
  }
                                                        ...
                                                        std::string    const&  instance)
{
  ...
  try{
    for(size_t i = begin_indx; i < end_indx; ++i){
      assn.addSingle(art::PtrMaker<T>{evt, instance}(i), b);
    }
  }
  catch(cet::exception &e){
    mf::LogWarning("AssociationUtil") << "unable to create requested "
							  ...
							  std::string    const&  instance)
{
  ...
  try{
    for(size_t i = 0; i < b.size(); ++i) assn.addSingle(art::PtrMaker<T>{evt, instance}(indx), b[i]); 
  }
  catch(cet::exception &e){
    mf::LogWarning("AssociationUtil") << "unable to create requested "
            << "art:Assns, exception thrown: "
                                                        ...
                                                        std::string     const& instanceb)
{
  ...
  try{
    for(size_t i = startU; i < endU; ++i){
      assn.addSingle(art::PtrMaker<T>{evt, instancea}(indx), art::PtrMaker<U>{evt, instanceb}(i));
    }
  }
  catch(cet::exception &e){
    mf::LogWarning("AssociationUtil") << "unable to create requested "
                                                        ...
                                                        std::string     const& instanceb)
{
  ...
  try{
    assn.addSingle(art::PtrMaker<T>{evt, instancea}(indxa), art::PtrMaker<U>{evt, instanceb}(indxb));
  }
  catch(cet::exception &e){
    mf::LogWarning("AssociationUtil") << "unable to create requested "
            << "art:Assns, exception thrown: "
