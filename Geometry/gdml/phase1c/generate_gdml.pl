...
$target_switch = 1;
@target_matt = ("Graphite", "CH2", "Be", "Aluminum", "Iron", "Water", "Air");
if ( defined $target) {
...
print DEF <<EOF;
	...
	<position name="T0_pos" z="T0_shift+T0_acrylic_shift" unit="mm"/>
...
print DEF <<EOF;
	...
	<position name="aerogel_pos0" x="0" y="0" z="aerogel_shift-0.5*arich_thick+0.5*aerogel_thick0" unit="mm" />
	<position name="aerogel_pos1" x="0" y="0" z="aerogel_shift-0.5*arich_thick+aerogel_thick0+0.5*aerogel_thick1" unit="mm" />
...
EOF
	...
	for($i = 0; $i < $nSSD_station; ++$i) {
print DEF <<EOF;
	<position name="ssdStation@{[ $i ]}_pos" x="0" y="0" z="ssdStation@{[ $i ]}_shift+ssdD0_thick-0.5*mount_thick" unit="mm" />
...
print DEF <<EOF;
	...
	<position name="ssd_bkpln_@{[ $i ]}_@{[ $j ]}_@{[ $k ]}_pos" x="$txs" y="$tys" z="$j*ssdD0_thick+($j-0.5)*mount_thick+($j-0.5)*ssd_bkpln_thick+$SSD_alignz{$idx}$tzs" unit="mm" />
...
print DEF <<EOF;
	<position name="ssdsensor_@{[ $i ]}_@{[ $j ]}_@{[ $k ]}_pos" x="$txs" y="$tys" z="0.5*ssdD0_thick+ssd_bkpln_thick+0.5*mount_thick+$SSD_alignz{$idx}$tzs" unit="mm" />
	<position name="ssd_bkpln_@{[ $i ]}_@{[ $j ]}_@{[ $k ]}_pos" x="$txs" y="$tys" z="0.5*ssd_bkpln_thick+0.5*mount_thick+$SSD_alignz{$idx}$tzs" unit="mm" />
	<rotation name="ssdsensor_@{[ $i ]}_@{[ $j ]}_@{[ $k ]}_rot" x="180.0*@{[ $SSD_side[$isensor] ]}" y="0" z="$tzrot" unit="deg"/>
EOF
	  	    	$isensor++;
  				}
...
print DEF <<EOF;
	<position name="ssdmount_local_@{[ $i ]}_@{[ $j ]}_pos" x="0" y="0" z="0" unit="mm"/>
	<position name="ssdmount_@{[ $i ]}_@{[ $j ]}_pos" x="0" y="0" z="$SSD_mount_shift[ $imount ]+0.5*mount_thick" unit="mm" />
...
print DEF <<EOF;
	...
	<position name="ssd_USMylarWindow_pos" x="0" y="0" z="-1*Mylar_shift" unit="mm" />
	<position name="ssd_DSMylarWindow_pos" x="0" y="0" z="Mylar_shift" unit="mm"/>

	<position name="ssd_mount_u1_pos" x="70" y="0" z="0" unit="mm"/>
	<rotation name="ssd_mount_u1_rot" y="-90" unit="deg"/>
	<position name="ssd_mount_u2_pos" x="0" y="-70" z="0" unit="mm"/>
	<rotation name="ssd_mount_u2_rot" y="-90" z="-90" unit="deg"/>
	<rotation name="ssd_mount_u3_rot" z="90" unit="deg"/>
	<position name="ssd_mount_u4_pos" x="80.1" y="-80.4" unit="mm"/>
...
EOF
  	...
  	for($i = 0; $i < $nD0chan; ++$i){
print DEF <<EOF;
	<position name="ssd_chan_@{[ $i ]}_pos" x="0" y="(@{[ -($nD0chan-1)/2 ]}+@{[ $i ]})*(ssdD0_chanwidth+ssdD0_changap)" z="0" unit="mm"/>
EOF
	}
...
print DEF <<EOF;
	<!-- BELOW IS FOR TARGET -->
	    ...
	    <position name="target_pos" x="0" y="0" z="380.5" unit="mm"/>
...
print DEF <<EOF;
    ...
    <position name="magnet_pos" x="0" y="0" z="magnetShift" unit="mm"/>
...
EOF
	 for($i = 0; $i < $n_RPC; ++$i){
print DEF <<EOF;
     <position name="RPC@{[ $i ]}_pos" z="RPC@{[ $i ]}_shift+0.5*RPC_thick-RPC_shift" unit="mm"/>
EOF
		 for($j = 0; $j < $n_cover; ++$j){
print DEF <<EOF;
     <position name="RPC@{[ $i ]}_Al@{[ $j ]}_pos" z="RPC_thick*($j-($n_cover-1)*0.5)+RPC@{[ $i ]}_shift+0.5*RPC_thick-RPC_shift" unit="mm"/>
EOF
	     }
     }
print DEF <<EOF;
...
print DEF <<EOF;
    ...
    <position name="LG_trap_pos1" x="0" y="0" z="-0.5*(LG_TransHorOff_shift+LG_length+LG_protrusion_thick+LG_PMTl-LG_length)" unit="mm" />
	<position name="LG_protrusion_pos" x="0" y="0" z="LG_length-0.5*(LG_TransHorOff_shift+LG_length+LG_protrusion_thick+LG_PMTl)+0.5*LG_protrusion_thick" unit="mm" />
	<position name="LG_PMT_pos" x="0" y="0" z="LG_length-0.5*(LG_TransHorOff_shift+LG_length+LG_protrusion_thick+LG_PMTl)+LG_protrusion_thick+0.5*LG_PMTl" unit="mm" />

EOF
	for($i = 0; $i < $n_LG; ++$i){
print DEF <<EOF;
	<position name="LG_block@{[ $i ]}0_pos" x="(0.5*(LG_width0+LG_width1T)+LG_TransHorOff_shift+Tolerance_space)" y="LG_height*($i-1)+2.5*($i-1)" z="0." unit="mm" />
	<position name="LG_block@{[ $i ]}1_pos" x="0" y="LG_height*($i-1)+2.5*($i-1)" z="0" unit="mm" />
	<position name="LG_block@{[ $i ]}2_pos" x="-(0.5*(LG_width0+LG_width1T)+LG_TransHorOff_shift+Tolerance_space)" y="LG_height*($i-1)+2.5*($i-1)" z="0." unit="mm" />
...
EOF
	...
	if($SSD_switch){
	 ...
	 <box name="ssd_bkpln_box" x="ssdD0_width" y="ssdD0_height" z="ssd_bkpln_thick" />

	 <box name="ssd_mount_box1" x="mount_width" y="mount_width" z="mount_thick" />
	 <box name="ssd_mount_box2" x="mount_hole" y="mount_hole" z="mount_thick" />
    <tube name="ssd_mount_tube" z="mount_thick" rmax="85.0" deltaphi="90" aunit="deg"/>
    <trd name="ssd_mount_trd" x1="mount_thick" x2="mount_thick" y1="115.98" y2="170.00" z="140"/>
    <trd name="ssd_mount_tri" x1="mount_thick" x2="mount_thick" y1="0" y2="115.98" z="57.99"/>

    <union name="ssd_mount_u1">
	   <first ref="ssd_mount_box1"/>
		<second ref="ssd_mount_trd"/>
		<positionref ref="ssd_mount_u1_pos"/>
	   <rotationref ref="ssd_mount_u1_rot"/>
	 </union>
	 <union name="ssd_mount_u2">
		<first ref="ssd_mount_u1"/>
		<second ref="ssd_mount_trd"/>
		<positionref ref="ssd_mount_u2_pos"/>
	   <rotationref ref="ssd_mount_u2_rot"/>
	 </union>
	 <union name="ssd_mount_u3">
		<first ref="ssd_mount_u2"/>
		<second ref="ssd_mount_tube"/>
	   <rotationref ref="ssd_mount_u3_rot"/>
	 </union>
	 <union name="ssd_mount_u4">
	   <first ref="ssd_mount_u3"/>
		<second ref="ssd_mount_tri"/>
		<positionref ref="ssd_mount_u4_pos"/>
	   <rotationref ref="ssd_mount_u4_rot"/>
	 </union>

	 <subtraction name="ssd_mount_box">
	   <first ref="ssd_mount_u4"/>
	 ...
	 <union name="ssd_mount_enclosure_u1">
	   <first ref="ssd_mount_box"/>
		...
		<positionref ref="ssd_mount_out_pos1"/>
	 </union>
	 <union name="ssd_mount_enclosure_box">
	   <first ref="ssd_mount_enclosure_u1"/>
		<second ref="ssd_mount_out_box"/>
		<positionref ref="ssd_mount_out_pos2"/>
	 </union>
...
EOF
		...
		for($i = 0; $i < $nSSD_station; ++$i){
			for($j = 0; $j < $SSD_lay[$SSD_station[$i]]; ++$j){
				for($k = 0; $k < $SSD_par[$SSD_station[$i]]; ++$k){
					print MOD <<EOF;
		 <volume name="ssdsensor_@{[ $i ]}_@{[ $j ]}_@{[ $k ]}_vol">
			<materialref ref="SiliconWafer"/>
...
EOF
		...
		for($i = 0; $i < $nSSD_station; ++$i){
			...
			for($j = 0; $j < $SSD_mount[ $SSD_station[ $i ] ]; ++$j){
				print DET <<EOF;
	<volume name="ssd_mount_enclosure_@{[ $i ]}_@{[ $j ]}_vol">
		<materialref ref="Air"/>
		<solidref ref="ssd_mount_enclosure_box"/>
EOF
				if ($j ==0) {
					$nlayer = 2;
				}
				...
				for($l = 0; $l < $nlayer; ++$l){
                                       	for($k = 0; $k < $SSD_par[$SSD_station[$i]]; ++$k){
						print DET <<EOF;
		<physvol name="ssdsensor_@{[ $i ]}_@{[ $ilayer ]}_@{[ $k ]}_phys">
			<volumeref ref="ssdsensor_@{[ $i ]}_@{[ $ilayer ]}_@{[ $k ]}_vol"/>
			<positionref ref="ssdsensor_@{[ $i ]}_@{[ $ilayer ]}_@{[ $k ]}_pos"/>
			<rotationref ref="ssdsensor_@{[ $i ]}_@{[ $ilayer ]}_@{[ $k ]}_rot"/>
		 </physvol>

		 <physvol name="ssd_bkpln_@{[ $i ]}_@{[ $ilayer ]}_@{[ $k ]}_phys">
			<volumeref ref="ssd_bkpln_vol"/>
			<positionref ref="ssd_bkpln_@{[ $i ]}_@{[ $ilayer ]}_@{[ $k ]}_pos"/>
			<rotationref ref="ssdsensor_@{[ $i ]}_@{[ $ilayer ]}_@{[ $k ]}_rot"/>
		 </physvol>
EOF
					}
...
EOF
	  ...
	  <volume name="ssdStation@{[ $station_type[$SSD_station[$i]] ]}@{[ $i ]}_vol">
		 ...
		 <physvol name="ssd@{[ $station_type[$SSD_station[$i]] ]}@{[ $i ]}_USMylarWindow_phys">
			...
			<positionref ref="ssd_USMylarWindow_pos"/>
		 </physvol>
		 <physvol name="ssd@{[ $station_type[$SSD_station[$i]] ]}@{[ $i ]}_DSMylarWindow_phys">
			<volumeref ref="ssd@{[ $station_type[$SSD_station[$i]] ]}_MylarWindow_vol"/>
			<positionref ref="ssd_DSMylarWindow_pos"/>
		 </physvol>

EOF
			for($j = 0; $j < $SSD_mount[$SSD_station[$i]]; ++$j){
					print DET <<EOF;
		 <physvol name="ssd_mount_enclosure_@{[ $i ]}_@{[ $j ]}_phys">
			<volumeref ref="ssd_mount_enclosure_@{[ $i ]}_@{[ $j ]}_vol"/>
			<positionref ref="ssdmount_@{[ $i ]}_@{[ $j ]}_pos"/>
		 </physvol>
EOF
