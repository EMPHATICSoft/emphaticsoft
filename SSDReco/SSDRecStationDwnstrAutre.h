  ...
  namespace ssdr {
    ...
    class SSDRecStationDwnstrAutre {
	 ...
	 explicit SSDRecStationDwnstrAutre(size_t aStationnum); // No args .. for now.. 
	  ...
	  //
          std::vector<rb::SSDStationPtAutre> fStPoints; // the stuff this routine produces. 
	  ...
	  // Internal stuff.. 
	  int fIdStPtNow; // Identifying the SSDStationPtAutre objects.
...
//	    std::cerr << " ... SSDRecStationDwnstrAutre::SetCoeffsAlignUncert, x " << x << " y " << y << " for station " << fStationNum << std::endl;
	 ...
	 inline std::vector<rb::SSDStationPtAutre>::const_iterator CBegin() const { return fStPoints.cbegin(); } 
	 inline std::vector<rb::SSDStationPtAutre>::const_iterator CEnd() const { return fStPoints.cend(); } 
	 inline rb::SSDStationPtAutre GetStPoint(std::vector<rb::SSDStationPtAutre>::const_iterator it) const { return *it; } 
	 inline rb::SSDStationPtAutre GetStPoint(size_t i) const { return fStPoints[i]; } 
	 // Deep copy, but small struct..
	 inline std::vector<rb::SSDStationPtAutre>::const_iterator GetStationPointPtr(int id) const { 
	   for (std::vector<rb::SSDStationPtAutre>::const_iterator itSt = fStPoints.cbegin(); itSt != fStPoints.cend(); itSt++) { 
	 ...
	 inline void ResetUsage() const { // pseudo const, not part of this reconstruction. For tracking usage.. 
	   for (std::vector<rb::SSDStationPtAutre>::const_iterator it = fStPoints.cbegin(); it != fStPoints.cend(); it++) it->SetUserFlag(0);
	 }
	 ...
	 int NumTriplets() const  {
	   int n = 0;
	   for (std::vector<rb::SSDStationPtAutre>::const_iterator it = fStPoints.cbegin(); it != fStPoints.cend(); it++) {
