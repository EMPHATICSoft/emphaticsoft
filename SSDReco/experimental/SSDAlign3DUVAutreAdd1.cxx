...
namespace emph {
  namespace ssdr {
     void SSDAlign3DUVAutre::writeNominalCoords() {
    ...
    void SSDAlign3DUVAutre::dumpCompactEvt(int spill, int evt, bool strictY6St, bool strictX6St, 
                                           const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
      if (!fFOutCompact.is_open()) {
       std::ostringstream aFOutCompactStrStr; 
       if ((!strictY6St) && (!strictX6St)) aFOutCompactStrStr << "./CompactAutreData_" << fRunNum  << "_" << fTokenJob << "_V1b.dat";
       else {
         if ((strictY6St) && (!strictX6St)) aFOutCompactStrStr << "./CompactAutreData_" << fRunNum  << "_" << fTokenJob << "_V1c2.dat";
         if ((strictX6St) && (!strictY6St)) aFOutCompactStrStr << "./CompactAutreData_" << fRunNum  << "_" << fTokenJob << "_V1d.dat";
//         if ((strictX6St) && (strictY6St)) aFOutCompactStrStr << "./CompactAutreData_" << fRunNum  << "_" << fTokenJob << "_V1e.dat";
// With track number 
         if ((strictX6St) && (strictY6St)) aFOutCompactStrStr << "./CompactAutreData_" << fRunNum  << "_" << fTokenJob << "_V1g.dat";
	...
	if (itCl->View() == geo::X_VIEW) {
	  ...
	  kStI = 2*(kSt-4) + kSe - 4;
	  if (kStI > 3) {
	    std::cerr << " SSDAlign3DUVAutre::dumpCompactEvt, inconsistent W view with station and sensor index " 
...
//        if (nClX[6] != 0) return; // We also veto the low multiplicity sensor, concentrated onsensor 5 and 7 of station 4 & 5. 
    ...
    //
    bool SSDAlign3DUVAutre::IsInPencilBeamRegion(std::vector<rb::SSDCluster>::const_iterator itCl) {
