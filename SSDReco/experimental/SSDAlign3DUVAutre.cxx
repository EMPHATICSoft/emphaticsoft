...
namespace emph {
  namespace ssdr {
     ...
     SSDAlign3DUVAutre::SSDAlign3DUVAutre() :
     ...
     void SSDAlign3DUVAutre::InitializeCoords(bool lastIs4, const std::vector<double> &zCoordXs, const std::vector<double> &zCoordYs,
	                                     const std::vector<double> &zCoordUs, const std::vector<double> &zCoordVs)
      {
     
       art::ServiceHandle<emph::geo::GeometryService> geo;
       if (geo->Geo() == nullptr) {
         std::cerr << " Problem in SSDAlign3DUVAutre::InitializeCoords, can not get the Geometry unique pointer... Fatal, quit now " << std::endl; exit(2);
       }
       fNoMagnet = (geo->Geo()->MagnetUSZPos() < 0.);
         if (lastIs4)  {
	  std::cerr << " SSDAlign3DUVAutre::InitializeCoords, unsupported option alt45, quit here and now " << std::endl;
	  exit(2);
       }
       if ((zCoordXs.size() != 8) || (zCoordYs.size() != 8)) {
         std::cerr << "  SSDAlign3DUVAutre::InitailizeCoords Unexpected number of ZCoords, " 
       ...
       if (zCoordUs.size() != 2) {
         std::cerr <<  " SSDAlign3DUVAutre::InitializeCoords,, unexpected number of U planes. should be 2 of them, we have " 
	           <<  zCoordUs.size()  << " fatal, here and now " << std::endl;
		   exit(2);
       }
       if (zCoordVs.size() != 4) {
         std::cerr <<  " SSDAlign3DUVAutre::InitializeCoords,, unexpected number of V planes. should be 4 of them, we have " 
     ...
     void ssdr::SSDAlign3DUVAutre::SetForMomentum(double p) {
       if (fMomentumIsSet) {
     ...
     void ssdr::SSDAlign3DUVAutre::openOutputCsvFiles() {
       //
     ...
     bool ssdr::SSDAlign3DUVAutre::recoXY(emph::geo::sensorView theView, const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
     
//
        if ((theView != emph::geo::X_VIEW) && (theView != emph::geo::Y_VIEW)) {
	  std::cerr << " SSDAlign3DUVAutre::recoXY, unexpected view enum emph::geo::sensorView " << theView << " expect X or Y, fatal, I said it " << std::endl;
	  exit(2);
	}
	...
	if (debugIsOn && (theView == emph::geo::X_VIEW)) std::cerr << " SSDAlign3DUVAutre::recoXY. X view, at event " 
	                         << fEvtNum << " number of clusters " << nHitsTotal <<  std::endl;
	if (debugIsOn && (theView == emph::geo::Y_VIEW)) std::cerr << " SSDAlign3DUVAutre::recoXY. Y view, at event " 
	                         << fEvtNum << " number of clusters " << nHitsTotal <<  std::endl;
//      
...
//	     return false; See blow.. 
	...
	for (int i2 = 0; i2 != numMaxH2345; i2++) {
     ...
     int ssdr::SSDAlign3DUVAutre::recoUV(emph::geo::sensorView view, const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
       ...
       if (debugIsOn) std::cerr << " SSDAlign3DUVAutre::RecoUV, view Index " << view  
     ...
     bool ssdr::SSDAlign3DUVAutre::fit3D(size_t minNumHits) {
       
//       bool debugIsOn = ( fEvtNum == 5 );  
       bool debugIsOn = false;  
     
       if (debugIsOn) std::cerr << " SSDAlign3DUVAutre::fit3D, evt " << fEvtNum <<  " min num hits " 
	 ...
	 std::cerr << " ----------------- " << std::endl;  std::cerr << std::endl;
       ...
       if (myNonLin3DFitPtr == nullptr) {
          myNonLin3DFitPtr = new SSD3DTrackFitFCNAutre(fRunNum);
...
//         std::cerr << " chi-Square Once.. " << aChiSqOnce << " .... and that is enough for now " << std::endl; exit(2);
     ...
     bool ssdr::SSDAlign3DUVAutre::checkUV(emph::geo::sensorView view, size_t kStation, 
                                           const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
     
       bool debugIsOn = (fEvtNum < 25);    
     
       if (debugIsOn) std::cerr << " SSDAlign3DUVAutre::checkUV, view Index " << view << " Station " << kStation 
                 << " nunH " << aSSDClsPtr->size() << "  working on it.. " << std::endl;
	
       if (kStation < 2) {
         std::cerr << " SSDAlign3DUVAutre::checkUV, " << view << " Unexpected Station number = " << kStation << " fatal mistake! " << std::endl; 
	 exit(2);
       } 
	 ...
	 default:
	 { 
           std::cerr << " SSDAlign3DUVAutre::checkUV, Unexpected " << view << " Station number = " << kStation << " fatal mistake! " << std::endl; 
	   exit(2);
         } 
       }
     ...
     void  ssdr::SSDAlign3DUVAutre::alignIt(const art::Event &evt, const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
       ...
       bool debugIsOn = fEvtNum == 5;
       if (debugIsOn) std::cerr << " SSDAlign3DUVAutre::alignIt, at event " << fEvtNum << std::endl; 
       fDataFor3DFit.clear();
...
//       if (gotX || gotY) this->dumpXYInfo(static_cast<int>(aSSDClsPtr->size())); Move to RecoBeamTrack
       if (gotX && gotY) {
         int numUVCheck = 0; 
	 ...
	 if (fDo3DFit) {
       ...
       if(fEvtNum > 250000000) {
         std::cerr << " ssdr::SSDAlign3DUVAutre::alignIt, quit here and at event " << fEvtNum << " quit now ! " << std::endl;
	 exit(2); 
       }
     }
   } // ssdr 
