...
namespace emph { 
  namespace ssdr {
    ...
    const double SSDRecBrickTracksAutre::fSqrt2 = std::sqrt(2.0);
    const double SSDRecBrickTracksAutre::fOneOverSqrt2 = std::sqrt(0.5); 
				
    SSDTrPreArbitrationAutre::SSDTrPreArbitrationAutre(int itId, int ix0, int iy0, int ix1, 
	...
	for (size_t kkSt=0; kkSt != 4; kkSt++) { // Phase1b 
	    SSDRecStationDwnstrAutre aStRec(kkSt+2);
    ...
    SSDRecBrickTracksAutre::~SSDRecBrickTracksAutre() {
      ...
      std::cerr << " SSDRecBrickTracksAutre destructor, number of saved Compact events for alignment " << fNumCompactSaved << std::endl;
      delete fFitterFCN;
      std::cerr << " SSDRecBrickTracksAutre destructor, and done !  " << std::endl;
    }
    //
    size_t SSDRecBrickTracksAutre::RecStation(size_t kSt, const art::Event &evt, 
                                const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
	
	if ((kSt < 2) || ((kSt-2) > fInputStPts.size())) { // Phase1b 
	  std::cerr << " SSDRecBrickTracksAutre::RecStation, illegal station number kSt " << kSt << " fatal... I mean it.. " << std::endl;
	  exit(2);
	}
	return fInputStPts[kSt-2].RecIt(evt, aSSDClsPtr);			
    }
    // 
    size_t SSDRecBrickTracksAutre::RecAndFitIt(const art::Event &evt, 
                const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
      ...
      if (fDebugIsOn) std::cerr << " SSDRecBrickTracksAutre::RecIt, starting on spill " 
	 ...
	 if (!fNoMagnet) {
	   std::cerr << " SSDRecBrickTracksAutre::RecAndFitIt ..The magnet seems to be in place, should not be.. " 
...
//        this->dumpInfoForR(); done in the main art module 
    ...
    //
    size_t SSDRecBrickTracksAutre::RecAndFitAll6Stations(const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
...
//
			...
			//
			SSDTrPreArbitrationAutre aNewTrAA(itNewTr->ID(), x0Indices[kx0], y0Indices[ky0], x1Indices[kx1], y1Indices[ky1], 
      ...
      if (fDebugIsOn) std::cerr << " End of SSDRecBrickTracksAutre::RecAndFitAll6Stations, num Tracks " << fTrs.size() << std::endl;
      return fTrs.size(); 
    }
    //
...
//       fFitterFCN->SetDebugOn(fDebugIsOn);
       ...
       if (fDebugIsOn) {
         std::cerr << " SSDRecBrickTracksAutre::doFitAndStore, start, X0 = " <<  xStart << " Y0 " << yStart 
...
//         std::cerr << " chi-Square Once.. " << aChiSqOnce << " .... and that is enough for now " << std::endl; exit(2);
    ...
    size_t SSDRecBrickTracksAutre::Arbitrate() {
      if (fDebugIsOn)  std::cerr << " Entering SSDRecBrickTracksAutre::Arbitrate, spill " 
                                 << fSubRunNum << " evt " << fEvtNum << " with " << fTrs.size() << " tracks " << std::endl;
      if (fTrs.size() < 2 ) return fTrs.size();
      if (fTrsForArb.size() == 0) {
        std::cerr << " SSDRecBrickTracksAutre::Arbitrate Logic error, no back pointer information, fatal, quit here and now " << std::endl;
	exit(2); 
      }
      if (fTrsForArb.size() < 2 ) {
        std::cerr << " SSDRecBrickTracksAutre::Arbitrate Logic error, t enough  back pointer information, fatal...  " << std::endl;
	exit(2); 
      }
    ...
    void SSDRecBrickTracksAutre::openOutputCsvFiles() const {
       //
       // only the UV matching info is here... 
       //
       
       std::ostringstream fNameStrStr;
       fNameStrStr << "SSDRecBrickTracksAutre_Run_" << fRunNum << "_" << fTokenJob << "_V1.txt";
     ...
     //
     void SSDRecBrickTracksAutre::dumpInfoForR() const {
     ...
     void SSDRecBrickTracksAutre::dumpCompactEvt(const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
       ...
       if (fDebugIsOn) std::cerr << " SSDRecBrickTracksAutre::dumpCompactEvt, at spill " << fSubRunNum << " evt " << fEvtNum << std::endl;
       int key=687503; // fixed, distinct from similar code found in SSDAlign3DUVAutreAdd
	 ...
	 for (size_t kkSt = 0; kkSt != fInputStPts.size(); kkSt++) { // on downstream stations (with respect to empty target !) 
	   ...
	   switch (kSt) {
	     ...
	     default : {
	       std::cerr << "  SSDRecBrickTracksAutre::dumpCompactEvt Space Point Station indices error, kSt " 
	   ...
	   if (itSt->Station() != static_cast<int>(kSt)) {
	     std::cerr << "  SSDRecBrickTracksAutre::dumpCompactEvt Space Point Station number indices error, kSt " 
	 ...
	 fNumCompactSaved++;
	 std::cerr << " SSDRecBrickTracksAutre::dumpCompactEvt, saving alignment event " << fEvtNum << std::endl;
	 ...
	 int trUID = itTr->ID(); 
         fFOutCompact.write(reinterpret_cast<char*>(&trUID), sizeof(int)); // Additional word, compare to first version in SSDAlign3DUVAutreAdd.cxx
     ...
     // 
     bool SSDRecBrickTracksAutre::isClusterIsolated(std::vector<rb::SSDCluster>::const_iterator itClSel, 
