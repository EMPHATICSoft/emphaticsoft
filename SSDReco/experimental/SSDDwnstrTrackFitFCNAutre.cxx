...
namespace emph {
  namespace ssdr {
    ...
    SSDDwnstrTrackFitFCNAutre::SSDDwnstrTrackFitFCNAutre(int runNum) :
    FCNBase(),
    fRunNum(runNum),
    ...
    fZLocUpstreamMagnet(DBL_MAX), fZLocDownstrMagnet(DBL_MAX) {
       ...
       std::cerr << " SSDDwnstrTrackFitFCNAutre::SSDDwnstrTrackFitFCNAutre, Magnet MagnetUSZPos() " <<  geo->Geo()->MagnetUSZPos() << std::endl;
       fPhase1c = (runNum > 1999) ? true : false;
//
    }
    SSDDwnstrTrackFitFCNAutre::~SSDDwnstrTrackFitFCNAutre() {
    ...
    double SSDDwnstrTrackFitFCNAutre::operator()(const std::vector<double> &pars) const {
      
      if (fDebugIsOn) std::cerr << "SSDDwnstrTrackFitFCNAutre::operator, Start, number of track parameters " 
...
//      if (fDebugIsOn) { std::cerr << " ......Chi Sq is " << chi2 << " And we stop here...  " << std::endl; exit(2);}
    ...
    void SSDDwnstrTrackFitFCNAutre::OpenOutResids(const std::string &fNameStr) {
    ...
    void SSDDwnstrTrackFitFCNAutre::SpitOutResids(int spill, int evtNum) {
      if (!fFOutResids.is_open()) return;
