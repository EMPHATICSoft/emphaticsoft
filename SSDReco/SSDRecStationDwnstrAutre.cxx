...
namespace emph {
  namespace ssdr {
    ...
    const double SSDRecStationDwnstrAutre::fSqrt2 = std::sqrt(2.0);
    const double SSDRecStationDwnstrAutre::fOneOverSqrt2= 1.0/std::sqrt(2.0);

    SSDRecStationDwnstrAutre::SSDRecStationDwnstrAutre(size_t kSt) :
	 ...
	 if ((fStationNum < 2) || (fStationNum > 6)) {
	      std::cerr << " SSDRecStationDwnstrAutre Station number, value " << fStationNum 
    ...
    //
     SSDRecStationDwnstrAutre::~SSDRecStationDwnstrAutre() {
       if (fFOutSt != nullptr) { 
     ...
     // 
     size_t SSDRecStationDwnstrAutre::RecIt(const art::Event &evt, const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
       fIdStPtNow = 0;
       if (fStationNum == INT_MAX) {
         std::cerr <<  " SSDRecStationDwnstrAutre::RecIt The station number for this instance is not defined.  ";
	 std::cerr << " One must call SetStationNumber.. Fatal " << std::endl; exit(2);
       }
       fRunNum = evt.run(); fSubRunNum = evt.subRun(); fEvtNum = evt.id().event();
       if (fDebugIsOn) {
           std::cerr << " SSDRecStationDwnstrAutre::RecIt, starting on spill " 
	...
	if (!fCoordConvert.IsReadyToGo()) {
	 ...
	 std::cerr << " SSDRecStationDwnstrAutre::RecIt, Station " << fStationNum << " Storing Z Positions ..." << std::endl; 
...
//	  exit(2);
     ...
     bool SSDRecStationDwnstrAutre::HasUniqueYW(const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) const {
       int nW=0; int nY=0;
       for(std::vector<rb::SSDCluster>::const_iterator itCl = aSSDClsPtr->cbegin(); itCl != aSSDClsPtr->cend(); itCl++) {
         if (itCl->Station() != fStationNum) continue;
     ...
     // Now the details..
     //
     size_t SSDRecStationDwnstrAutre::recoXYUorW(const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr){
       size_t kux = 0;
       std::vector<int> fClUsagesDbl(fClUsages); 
       size_t nStart = fStPoints.size();
       if (fDebugIsOn) {
          std::cerr << " Starting SSDRecStationDwnstrAutre::recoXYUorW, Station " << fStationNum 
...
//	     if (fDebugIsOn && (fStationNum == 5)) { std::cerr << "  check chiSqCut, and there it is " << std::endl; exit(2); }		      
     ...
     //
     size_t SSDRecStationDwnstrAutre::recoXY(const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
       size_t kux = 0;
       size_t nStart = fStPoints.size();
       const size_t kSt = static_cast<size_t>(fStationNum);
       if (fDebugIsOn) std::cerr << " Starting SSDRecStationDwnstrAutre::recoXY, Station " << fStationNum 
                        << "  with a total of " << aSSDClsPtr->size() << " clusters " << std::endl;
       for(std::vector<rb::SSDCluster>::const_iterator itClX = aSSDClsPtr->cbegin(); itClX != aSSDClsPtr->cend(); itClX++, kux++) {
         if (fClUsages[kux] != 0) continue;
         if (itClX->Station() != fStationNum) continue;
     ...
     size_t SSDRecStationDwnstrAutre::recoXUorW(const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
       size_t kux = 0;
       size_t nStart = fStPoints.size();
       const size_t kStMaxW = (fRunNum < 2000) ? 4 : 5;
       const size_t kSt = static_cast<size_t>(fStationNum);
       if (fDebugIsOn) std::cerr << " Starting SSDRecStationDwnstrAutre::recoXUorW, Station " << fStationNum 
                        << "  with a total of " << aSSDClsPtr->size() << " clusters " << std::endl;
       for(std::vector<rb::SSDCluster>::const_iterator itClX = aSSDClsPtr->cbegin(); itClX != aSSDClsPtr->cend(); itClX++, kux++) {
         if (fClUsages[kux] != 0) continue;
         if (itClX->Station() != fStationNum) continue;
     ...
     //
     size_t SSDRecStationDwnstrAutre::recoYUorW(const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
       size_t kuy = 0;
       size_t nStart = fStPoints.size();
       const size_t kStMaxW = (fRunNum < 2000)  ? 4 : 5;
       const size_t kSt = static_cast<size_t>(fStationNum);
       if (fDebugIsOn) std::cerr << " Starting SSDRecStationDwnstrAutre::recoYUorW, Station " << fStationNum 
                        << "  with a total of " << aSSDClsPtr->size() << " clusters " << std::endl;
       for(std::vector<rb::SSDCluster>::const_iterator itClY = aSSDClsPtr->cbegin(); itClY != aSSDClsPtr->cend(); itClY++, kuy++) {
         if (fClUsages[kuy] != 0) continue;
         if (itClY->Station() != fStationNum) continue;
    ...
    //
     void ssdr::SSDRecStationDwnstrAutre::openOutputCsvFiles() const {
       //
       // only the UV matching info is here... 
       //
       
       std::ostringstream fNameStrStr;
       fNameStrStr << "SSDRecStationDwnstrAutre_Run_" << fRunNum << "_" << fTokenJob << "_Station" << fStationNum << "_V1.txt";
       ...
       if (fDoFirstAndLastStrips) {
         ...
         fName2StrStr << "SSDRecStationDwnstrAutreY_FirstStrip_Run_" << fRunNum << "_" << fTokenJob << "_Station" << fStationNum << "_V1.txt";
         ...
         fName3StrStr << "SSDRecStationDwnstrAutreY_LastStrip_Run_" << fRunNum << "_" << fTokenJob << "_Station" << fStationNum << "_V1.txt";
         ...
         fName4StrStr << "SSDRecStationDwnstrAutreX_FirstStrip_Run_" << fRunNum << "_" << fTokenJob << "_Station" << fStationNum << "_V1.txt";
         ...
         fName5StrStr << "SSDRecStationDwnstrAutreX_LastStrip_Run_" << fRunNum << "_" << fTokenJob << "_Station" << fStationNum << "_V1.txt";
     ...
     void ssdr::SSDRecStationDwnstrAutre::dumpInfoForR() const {
       if ((fSubRunNum == 10) && (fEvtNum < 20)) 
             std::cerr << " ssdr::SSDRecStationDwnstrAutre::dumpInfoForR, Station " << fStationNum << " spill 10, evt " << fEvtNum << std::endl;
	 ...
	 for (size_t kCl=0; kCl != numClInSpSt; kCl++) {
		...
		default : { 
		  std::cerr << " ssdr::SSDRecStationDwnstrAutre::dumpInfoForR, unrecognized SSD Cluster.. keep going.. " << std::endl;
		}
	   }
     ...
     void ssdr::SSDRecStationDwnstrAutre::dumpInfoForRYViewEdges(const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) const {
     ...
     void ssdr::SSDRecStationDwnstrAutre::dumpInfoForRXViewEdges(const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) const {
       if (!fDoFirstAndLastStrips) return; 
       if (fStPoints.size() == 0) return;
       if (fFOutSt == nullptr)  this->openOutputCsvFiles(); 
       std::ostringstream headerStrStr; 
	...
	return false; 			  
     ...
     bool ssdr::SSDRecStationDwnstrAutre::hasOverlappingClusters (
	...
	return false; 			  
     ...
     bool ssdr::SSDRecStationDwnstrAutre::arbitrateOverlapping (double inViewCut) {
       if (fDebugIsOn) {
         std::cerr << " SSDRecStationDwnstrAutre::arbitrateOverlapping, on " << fStPoints.size() << " space points, cut " << inViewCut << std::endl;
       }
...
//	       const double yyErr = std::sqrt(1.0/sumW);
		   ...
		   itPt1->addOverlapClusters(itCls2[k2], itPt2->ClusterCorrMeasurement(1), itPt2->ClusterMeasurementError(1));
	           if (fDebugIsOn) {
	             std::cerr << " SSDRecStationDwnstrAutre::arbitrateOverlapping,  Removal of Station point Y View " << std::endl;
	             std::cerr << " " << (*itCls2[k2]) << std::endl; 
	            }
		 }
	       }
	     ...
	     if (this->flagDuplicateOverlapping(emph::geo::W_VIEW, itPt1, itPt2, inViewCut)) {
	       ...
	       std::vector<myItCl> itCls2; itPt2->fillItClusters(itCls2);
	         ...
	         if (itCls2[k2]->View() == emph::geo::W_VIEW) { 
	           if (fDebugIsOn) {
	             std::cerr << " SSDRecStationDwnstrAutre::arbitrateOverlapping,  Removal of Station point W View " << std::endl;
	             std::cerr << " " << (*itCls2[k2]) << std::endl; 
	            }
