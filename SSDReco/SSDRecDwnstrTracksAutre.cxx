  ...
  namespace ssdr {
    ...
    size_t SSDRecDwnstrTracksAutre::RecAndFitAll4Stations() 
    {
      ...
      if (fDebugIsOn) {
        ...
        for (std::vector<rbex::SSDStationPt>::const_iterator itSt2 = fInputSt2Pts.CBegin(); itSt2 != fInputSt2Pts.CEnd(); itSt2++)  
        std::cerr << " ID " << itSt2->ID() << " X = " << itSt2->X() << " Y " << itSt2->Y() << " ChiSq " << itSt2->ChiSq() << std::endl;
        std::cerr << " Dump of XY position for Station 3 " << std::endl;
        for (std::vector<rbex::SSDStationPt>::const_iterator itSt3 = fInputSt3Pts.CBegin(); itSt3 != fInputSt3Pts.CEnd(); itSt3++)  
        std::cerr << " ID " << itSt3->ID() << " X = " << itSt3->X() << " Y " << itSt3->Y() << " ChiSq " << itSt3->ChiSq() << std::endl;
        std::cerr << " Dump of XY position for Station 4 " << std::endl;
        for (std::vector<rbex::SSDStationPt>::const_iterator itSt4 = fInputSt4Pts.CBegin(); itSt4 != fInputSt4Pts.CEnd(); itSt4++)  
        std::cerr << " ID " << itSt4->ID() << " X = " << itSt4->X() << " Y " << itSt4->Y() << std::endl;
        std::cerr << " Dump of XY position for Station 5 " << std::endl;
        for (std::vector<rbex::SSDStationPt>::const_iterator itSt5 = fInputSt5Pts.CBegin(); itSt5 != fInputSt5Pts.CEnd(); itSt5++)  
        std::cerr << " ID " << itSt5->ID() << " X = " << itSt5->X() << " Y " << itSt5->Y() << " ChiSq " << itSt5->ChiSq() << std::endl;
        std::cerr << " Dump of XY position for Station 6 " << std::endl;
        for (std::vector<rbex::SSDStationPt>::const_iterator itSt6 = fInputSt6Pts.CBegin(); itSt6 != fInputSt6Pts.CEnd(); itSt6++)  
              ...
              for (std::vector<rbex::SSDStationPt>::const_iterator itSt6 = fInputSt6Pts.CBegin(); itSt6 != fInputSt6Pts.CEnd(); itSt6++) 
              {
                ...
                if (nComb > 10000) {
                  std::cerr << " Starting on RecAndFitAll4Stations, evt " << fEvtNum <<  " spill " << fSubRunNum
                            << " humongous combination, gave up " << std::endl;
                  fTrs.clear(); return 0;
                }
                if (itSt5->UserFlag() != 0) continue;
                ...
                bool goodPrelimPlus = this->doFitAndStore(rbex::FIVESTATIONDWN, x2Start, y2Start, xSlopeStart, ySlopeStart, fPrelimFitMomPlus, false);
                ...
                bool goodPrelimMinus = this->doFitAndStore(rbex::FIVESTATIONDWN, x2Start, y2Start, xSlopeStart, ySlopeStart, fPrelimFitMomMinus, false); 
                ...
                if (goodPrelimPlus || goodPrelimMinus) {
                  ...
                  if (this->doFitAndStore(rbex::FIVESTATIONDWN, x2Start, y2Start, xSlopeStart, ySlopeStart, fPrelimFitMom, true)) 
                  {
                    if (fTrs.rbegin()->ChiSq() < fChiSqCutPreArb) 
                    { 
                      int kTrCnt = static_cast<int>(fTrs.size());
                      if (fDebugIsOn) std::cerr << " ... ... ... ...  ... Setting Used flags on Space Point, kTrCnt " <<  kTrCnt << " St2 ID " << itSt2->ID() << " St3 " <<  itSt3->ID()
                      << " St4 " <<  itSt4->ID() << " St5 " <<  itSt5->ID() <<  " St6 " <<  itSt6->ID()  << std::endl; 
                    ...
                    // Do the Klm Fits.. 
                    //
                    std::vector<rbex::DwnstrTrack>::reverse_iterator itLast = fTrs.rbegin(); 
                    dataInKlm.clear();
        ...
        { 
          ...
          for (std::vector<rbex::SSDStationPt>::const_iterator itSt5 = fInputSt5Pts.CBegin(); itSt5 != fInputSt5Pts.CEnd(); itSt5++) 
          {
            ...
            for (std::vector<rbex::SSDStationPt>::const_iterator itSt6 = fInputSt6Pts.CBegin(); itSt6 != fInputSt6Pts.CEnd(); itSt6++) 
            {
              ...
              fFitterFCN->AddInputPt(itSt6);
              const double fPrelimFitMomPlus = std::abs(fPrelimMomentum); const double fPrelimFitMomMinus = -1.0*std::abs(fPrelimMomentum);
              bool goodPrelimPlus = this->doFitAndStore(rbex::STATION2356DWN, x2Start, y2Start, xSlopeStart, ySlopeStart, fPrelimFitMomPlus, false);
              const double  prelimFitMomPlus  = fPrelimFitMom; 
              const double prelimChiSqPlus = fPrelimFitChiSq;
              bool goodPrelimMinus = this->doFitAndStore(rbex::STATION2356DWN, x2Start, y2Start, xSlopeStart, ySlopeStart, fPrelimFitMomMinus, false); 
              ...
              if (goodPrelimPlus || goodPrelimMinus) 
              {
                ...
                {  // For now!!!   We are not interested in low momentum tracks.. 
                ...
                if (this->doFitAndStore(rbex::STATION2356DWN, x2Start, y2Start, xSlopeStart, ySlopeStart, fPrelimFitMom, true)) 
                {
	                if (fTrs.rbegin()->ChiSq() < fChiSqCutPreArb) 
                  { 
	  	              int kTrCnt = static_cast<int>(fTrs.size());
  	  	            if (fDebugIsOn) std::cerr << " ... ... ... ...  ... Setting Used flags on Space Point, kTrCnt " <<  kTrCnt << " St2 ID " << itSt2->ID() << " St3 " <<  itSt3->ID() <<  " St5 " <<  itSt5->ID() <<  " St6 " <<  itSt6->ID()  << std::endl; 
                    itSt2->SetUserFlag(kTrCnt); itSt3->SetUserFlag(kTrCnt); 
    ...
    size_t SSDRecDwnstrTracksAutre::RecAndFitStation2345() 
    {
      ...
      if (fDebugIsOn) 
      {
        std::cerr << "\nStarting on RecAndFitStation2345, evt " << fEvtNum << " dz25 = " << dz25 << " with "
              << fInputSt2Pts.Size() << " St2 Pts, " << fInputSt3Pts.Size() << " St3 Pts, "
              << fInputSt4Pts.Size() << " St4 Pts, " << fInputSt5Pts.Size() << " St5 Pts, "
              << fInputSt6Pts.Size() << " St6 Pts\n";

        std::cerr << "Dump of XY position for Station 2\n";
        for (std::vector<rbex::SSDStationPt>::const_iterator itSt2 = fInputSt2Pts.CBegin(); itSt2 != fInputSt2Pts.CEnd(); itSt2++) {
          std::cerr << "ID " << itSt2->ID() << " X = " << itSt2->X() << " Y = " << itSt2->Y() << " ChiSq " << itSt2->ChiSq() << '\n';
        }

        std::cerr << "Dump of XY position for Station 3\n";
        for (std::vector<rbex::SSDStationPt>::const_iterator itSt3 = fInputSt3Pts.CBegin(); itSt3 != fInputSt3Pts.CEnd(); itSt3++) {
          std::cerr << "ID " << itSt3->ID() << " X = " << itSt3->X() << " Y = " << itSt3->Y() << " ChiSq " << itSt3->ChiSq() << '\n';
        }

        std::cerr << "Dump of XY position for Station 4\n";
        for (std::vector<rbex::SSDStationPt>::const_iterator itSt4 = fInputSt4Pts.CBegin(); itSt4 != fInputSt4Pts.CEnd(); itSt4++) {
          std::cerr << "ID " << itSt4->ID() << " X = " << itSt4->X() << " Y = " << itSt4->Y() << " ChiSq " << itSt4->ChiSq() << '\n';
        }

        std::cerr << "Dump of XY position for Station 5\n";
        for (std::vector<rbex::SSDStationPt>::const_iterator itSt5 = fInputSt5Pts.CBegin(); itSt5 != fInputSt5Pts.CEnd(); itSt5++) {
          std::cerr << "ID " << itSt5->ID() << " X = " << itSt5->X() << " Y = " << itSt5->Y() << " ChiSq " << itSt5->ChiSq() << '\n';
        }

        std::cerr << "Start Looping...\n";
      }
      ...
      {
        const double x2Start = itSt2->X();
        const double y2Start = itSt2->Y();
        if (itSt2->UserFlag() != 0) continue;

        if (fDebugIsOn) std::cerr << "At station 2, at x = " << itSt2->X() << " Y = " << itSt2->Y() << '\n';

        for (std::vector<rbex::SSDStationPt>::const_iterator itSt3 = fInputSt3Pts.CBegin(); itSt3 != fInputSt3Pts.CEnd(); itSt3++) 
        {
          if (itSt3->UserFlag() != 0) continue;

          if (fDebugIsOn) std::cerr << "At station 3, at x = " << itSt3->X() << " Y = " << itSt3->Y() << '\n';

          for (std::vector<rbex::SSDStationPt>::const_iterator itSt4 = fInputSt4Pts.CBegin(); itSt4 != fInputSt4Pts.CEnd(); itSt4++) 
          {
            if (itSt4->UserFlag() != 0) continue;

            if (fDebugIsOn) std::cerr << "At station 4, at x = " << itSt4->X() << " Y = " << itSt4->Y() << '\n';
            ...
            for (std::vector<rbex::SSDStationPt>::const_iterator itSt5 = fInputSt5Pts.CBegin(); itSt5 != fInputSt5Pts.CEnd(); itSt5++) 
            {
              ...
              if (fDebugIsOn) std::cerr << "At station 5, at x = " << itSt5->X() << " Y = " << itSt5->Y() << '\n';

              const double x5End = itSt5->X();
              const double y5End = itSt5->Y();
              const double xSlopeStart = (x5End - x2Start) / dz25;
              const double ySlopeStart = (y5End - y2Start) / dz25;

              fFitterFCN->ResetInputPts();
              fFitterFCN->AddInputPt(itSt2);
              fFitterFCN->AddInputPt(itSt3);
              fFitterFCN->AddInputPt(itSt4);
              fFitterFCN->AddInputPt(itSt5);

              const double fPrelimFitMomPlus = std::abs(fPrelimFitMom);
              const double fPrelimFitMomMinus = -1.0 * std::abs(fPrelimFitMom);

              bool goodPrelimPlus = this->doFitAndStore(rbex::STATION2345DWN, x2Start, y2Start, xSlopeStart, ySlopeStart, fPrelimFitMomPlus, false);
              const double prelimFitMomPlus = fPrelimFitMom;
              const double prelimChiSqPlus = fPrelimFitChiSq;

              bool goodPrelimMinus = this->doFitAndStore(rbex::STATION2345DWN, x2Start, y2Start, xSlopeStart, ySlopeStart, fPrelimFitMomMinus, false);
              const double prelimFitMomMinus = fPrelimFitMom;
              const double prelimChiSqMinus = fPrelimFitChiSq;
              ...
              if (goodPrelimPlus || goodPrelimMinus) 
              {
                ...
                if (!goodPrelimPlus && goodPrelimMinus) fPrelimFitMom = prelimFitMomMinus;
                if (goodPrelimPlus && goodPrelimMinus) {
                  fPrelimFitMom = (prelimChiSqPlus < prelimChiSqMinus) ? prelimFitMomPlus : prelimFitMomMinus;
                }

                if (std::abs(fPrelimFitMom) < 0.5) {
                  if (fDebugIsOn) std::cerr << "Momentum too low... skip that one\n";
                  continue;
                }
                ...
                if (fDebugIsOn) {
                  std::cerr << "At station 5, after Prelim fit, fPrelimMomentum " << fPrelimMomentum
                      << " fPrelimFitMom " << fPrelimFitMom << " XErr " << itSt5->XErr()
                      << " YErr " << itSt5->YErr() << '\n';
                }
                ...
                if (this->doFitAndStore(rbex::STATION2345DWN, x2Start, y2Start, xSlopeStart, ySlopeStart, fPrelimFitMom, true)) 
                {
                  if (fTrs.rbegin()->ChiSq() < fChiSqCutPreArb) 
                  {
                    int kTrCnt = static_cast<int>(fTrs.size());
                    if (fDebugIsOn) {
                      std::cerr << "Setting Used flags on Space Point, kTrCnt " << kTrCnt
                          << " St2 ID " << itSt2->ID() << " St3 " << itSt3->ID()
                          << " St4 " << itSt4->ID() << " St5 " << itSt5->ID() << '\n';
                    }

                    itSt2->SetUserFlag(kTrCnt);
                    itSt3->SetUserFlag(kTrCnt);
      ...
      if (fDebugIsOn) std::cerr << "End of SSDRecDwnstrTracksAutre::RecAndFitStation2345, num Tracks " << fTrs.size() << '\n';
        return fTrs.size();
    } // RecAndFitStation2345
    ...
    size_t SSDRecDwnstrTracksAutre::RecAndFitStation234()
    {
      ...
      if (fDebugIsOn) std::cerr << " Starting on RecAndFit,Station234 dz24 = " << dz24 << " with " 
                                << fInputSt2Pts.Size() << " St2 Pts,  " << fInputSt3Pts.Size() << " St3 Pts, "  << fInputSt4Pts.Size() << " St4 Pts,  " << std::endl;
      for (std::vector<rbex::SSDStationPt>::const_iterator itSt2 = fInputSt2Pts.CBegin(); itSt2 != fInputSt2Pts.CEnd(); itSt2++) 
      { 
        const double x2Start = itSt2->X(); 
        const double y2Start = itSt2->Y(); // To estimate starting value for the track param fit. 
        ...
        { 
          ...
          { 
            ...
            fFitterFCN->AddInputPt(itSt3);
            fFitterFCN->AddInputPt(itSt4);
	          if (this->doPrelimFit(rbex::FOURSTATION, x2Start, y2Start, xSlopeStart, ySlopeStart)) 
            {
              ...
              fFitterFCN->AddInputPt(itSt4); 
	            if (this->doFitAndStore(rbex::FOURSTATION, x2Start, y2Start, xSlopeStart, ySlopeStart, fPrelimFitMom)) 
              {
	              if (fTrs.rbegin()->ChiSq() < fChiSqCutPreArb) 
                { 
	   	            int kTrCnt = static_cast<int>(fTrs.size());
  	              ...
  	              if (fDebugIsOn) std::cerr << " ... ... ... ...  ... Setting Used flags on Space Point, kTrCnt " <<  kTrCnt << " St2 ID " << itSt2->ID() << " St3 " <<  itSt3->ID() << " St4 " <<  itSt4->ID() <<  std::endl; 
	              } 
	            }
		          // One must restore the uncertainties... The whole thing is ugly.. 
              itSt2->ReScaleMultUncert(multScattErrStation2, fPrelimFitMom, fPrelimMomentum); 
    ...
    size_t SSDRecDwnstrTracksAutre::RecAndFitStation235() {
      ...
      if (fDebugIsOn) std::cerr << " Starting on RecAndFit,Station235 dz26 = " << dz26 << " with " 
                               << fInputSt2Pts.Size() << " St2 Pts,  " << fInputSt3Pts.Size() << " St3 Pts, " << fInputSt5Pts.Size() << " St5 Pts,  " << std::endl;
      for (std::vector<rbex::SSDStationPt>::const_iterator itSt2 = fInputSt2Pts.CBegin(); itSt2 != fInputSt2Pts.CEnd(); itSt2++) 
      { 
        const double x2Start = itSt2->X(); 
        const double y2Start = itSt2->Y(); // To estimate starting value for the track param fit. 
         ...
         { 
            ...
            { 
              ...
              if (this->doPrelimFit(rbex::FOURSTATION, x2Start, y2Start, xSlopeStart, ySlopeStart)) 
              {
                ...
                itSt5->ReScaleMultUncert(multScattErrStation5, fPrelimMomentum, fPrelimFitMom);
                itSt6->ReScaleMultUncert(multScattErrStation6, fPrelimMomentum, fPrelimFitMom);
	              if (fDebugIsOn) std::cerr << " ... ... ... At station 6, after Prelim fit, fPrelimMomentum " << fPrelimMomentum << " fPrelimFitMom " << fPrelimFitMom << " XErr " << itSt6->XErr() << " YErr " << itSt6->YErr() << std::endl;
                fFitterFCN->AddInputPt(itSt2); fFitterFCN->AddInputPt(itSt3);
                fFitterFCN->AddInputPt(itSt5); fFitterFCN->AddInputPt(itSt6); 
                if (this->doFitAndStore(rbex::FOURSTATION, x2Start, y2Start, xSlopeStart, ySlopeStart, fPrelimFitMom)) 
                {
                  ...
                  { 
                    ...
                    itSt5->SetUserFlag(kTrCnt); itSt6->SetUserFlag(kTrCnt);
  	                if (fDebugIsOn) std::cerr << " ... ... ... ...  ... Setting Used flags on Space Point, kTrCnt " <<  kTrCnt << " St2 ID " << itSt2->ID() << " St2 " <<  itSt3->ID() << " St5 " <<  itSt5->ID() <<  " St6 " <<  itSt6->ID() <<std::endl; 
	                } 
	              }
		            // One must restore the uncertainties... The whole thing is ugly.. 
                itSt2->ReScaleMultUncert(multScattErrStation2, fPrelimFitMom, fPrelimMomentum); 
    ...
    //
    bool SSDRecDwnstrTracksAutre::doPrelimFit(rbex::DwnstrTrType type, double xStart, double yStart, double xSlopeStart, double ySlopeStart) {
       fNoMagnet = fEmgeo->MagnetUSZPos() < 0.;
...
//       if (fEvtNum == 5) { std::cerr << " SSDRecDwnstrTracksAutre::doPrelimFit Event 5, quit here.. " << std::endl; exit(2); }
    ...
    //
    bool SSDRecDwnstrTracksAutre::doFitAndStore(rbex::DwnstrTrType type, double xStart, double yStart, 
...
//       }
    ...
    //
    bool SSDRecDwnstrTracksAutre::IsAlreadyFound(const rbex::DwnstrTrack &aTr) const {  // this needs tuning.. 
    ...
    void SSDRecDwnstrTracksAutre::openOutputCsvFiles() const {
	 ...
	 fFOutCompactInfo << std::endl;
	  ...
	  x2 = DBL_MAX; y2 = DBL_MAX; nCl2 = 0;
	    ...
	    const rbex::SSDStationPt aPtSt2 = fInputSt2Pts.GetStPoint(i2); x2 = aPtSt2.X(); y2 = aPtSt2.Y(); nCl2 = aPtSt2.NumClusters();
...
//	      if (fDebugIsOn) std::cerr << " ... setting Cluster " << (*itCl) << std::endl; 
	    ...
	    x3 = DBL_MAX; y3 = DBL_MAX; nCl3 = 0;
	      ...
	      const rbex::SSDStationPt aPtSt3 = fInputSt3Pts.GetStPoint(i3); x3 = aPtSt3.X(); y3 = aPtSt3.Y(); nCl3 = aPtSt3.NumClusters();
...
//	        if (fDebugIsOn) std::cerr << " ... ... setting Cluster " << (*itCl) << std::endl; 
	      ...
	      x4 = DBL_MAX; y4 = DBL_MAX; nCl4 = 0;
              if (fInputSt4Pts.Size() != 0) {
	        const rbex::SSDStationPt aPtSt4 = fInputSt4Pts.GetStPoint(i4); x4 = aPtSt4.X(); y4 = aPtSt4.Y(); nCl4 = aPtSt4.NumClusters();
...
//	            if (fDebugIsOn) std::cerr << " ... ... ... setting Cluster " << (*itCl) << std::endl; 
	        ...
	        x5 = DBL_MAX; y5 = DBL_MAX; nCl5 = 0;
	          ...
	          const rbex::SSDStationPt aPtSt5 = fInputSt5Pts.GetStPoint(i5); x5 = aPtSt5.X(); y5 = aPtSt5.Y(); nCl5 = aPtSt5.NumClusters();
		 ...
		 for (size_t k=0; k!=itClSt5.size(); k++) { 
		   ...
		   if (itClSt5[k]->View() == emph::geo::W_VIEW) mySSDClsPtrsW[static_cast<size_t>(itClSt5[k]->Sensor())] = itClSt5[k]; 		   }
	          ...
	          x6 = DBL_MAX; y6 = DBL_MAX; nCl6 = 0;
	           ...
	           const rbex::SSDStationPt aPtSt6 = fInputSt6Pts.GetStPoint(i6); x6 = aPtSt6.X(); y6 = aPtSt6.Y(); nCl6 = aPtSt6.NumClusters();
