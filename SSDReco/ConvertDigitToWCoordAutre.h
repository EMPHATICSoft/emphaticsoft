...
namespace emph { 
  namespace ssdr {
    ...
    class ConvertDigitToWCoordAutre {
    
       public:
      
	explicit ConvertDigitToWCoordAutre(char aView); // X or Y, U or V,  or All of them... 
	explicit ConvertDigitToWCoordAutre(int aRunNum, char aView); // To support both Phase1b and Phase1c 
...
//
	...
	public:
	 ...
	 inline void SetCoeffsAlignUncert(double x, double y) { 
	    std::cerr << " ... ConvertToDigitWCoordAutre::SetCoeffsAlignUncert, x " << x << " y " << y << std::endl;
	    fXCoeffAlignUncert = x;  fYCoeffAlignUncert = y; 
	 }
	 inline void SetDownstreamGap(size_t aStation, double v) { 
	   if ((aStation != 4) && (aStation != 5)) {
	     std::cerr << " ConvertDigitToWCoordAutre, Station  " << aStation << " has no double sensor, fatal, quit here " << std::endl; 
	     exit(2);
	   } 
	 ...
	 inline void SetTheView(char aView) {
	   ...
	   if ((aView != 'X') && (aView != 'Y') && (aView != 'U')  && (aView != 'V')) {
	     std::cerr << " ConvertDigitToWCoordAutre, setting an unknown view " << aView << " fatal, quit here " << std::endl; 
	     exit(2);
	   }
	   fView = aView;
	 }
	 ...
	 inline double GetTsFromCluster(size_t kStation, size_t kPlane, double strip) {
	   std::cerr << " ConvertDigitToWCoordAutre::GetTsFromCluster Obsolete coordinate setting, quit here and now !! " << std::endl; exit(2);
...
//   When using the Monte-Carlo, and possibly the data , we had a sign mistake, before March 23 2023 	       
		 ...
		 double aVal =  (strip*fPitch + fNominalOffsets[kStation]  + 1.0e-10);
	      ...
	      case 'V' :// Station 4 and 5, double sensors.  
	      {
	        if (!fIsPhase1c) {
	          if (kStation < 4) {
	            std::cerr << " SSDAlign3DUVAutre::getTsFromCluster, unexpected station " << kStation 
		              << " for W view internal error, fatal " << std::endl; exit(2);
		  }
	          if (kPlane < 4) {
	            std::cerr << " SSDAlign3DUVAutre::getTsFromCluster, unexpected sensor " << kPlane 
		              << " for W view internal error, fatal " << std::endl; exit(2);
		  }
		} else { 
		 // 
		...
		// 
	      ...
	      default :
	        std::cerr << " ConvertDigitToWCoordAutre::getTsFromCluster, unexpected view, "
...
//
// New version... To be included in SSD3DTrackFitFCNAutre, at some point... Or, this entire class could go away.. 
...
#endif // ConvertDigitToWCoordAutre
