...
namespace emph { 
  namespace ssdr {
    ...
    bool SSDRecDwnstrTracksAutre::doUpDwn3DClFitAndStore( double pStart) {
      if (fRunNum < 2000) return false; // Disable Phase1b . 
      std::cerr << " ... SSDRecDwnstrTracksAutre::doUpDwn3DClFitAndStore Need to be re-comissioned... Fatal...  " << std::endl; exit(2); 
...
//      if (fInputSt5Pts.NumTriplets() != 1) return false;
      ...
      for (size_t k=0; k != itClsTmp.size(); k++)  dataIn.push_back(itClsTmp[k]);
      //
       if (fDebugIsOn) std::cerr << " SSDRecDwnstrTracksAutre::doUpDwn3DClFitAndStore, evt " 
	 ...
	 std::cerr << " ----------------- " << std::endl;  std::cerr << std::endl;
       ...
       if (fUpDownFitterFCN == nullptr) {
          fUpDownFitterFCN = new SSD3DTrackFitFCNAutre(fRunNum);
...
//	  (*fUpDownFitterFCN)(parsOut);
	 ...
	 // Covariance matrices needs to be studied. 
      ...
      // 
//       if (fDebugIsOn) { 
//          std::cerr << " .. O.K., enough of SSDRecDwnstrTracksAutre::doUpDwn3DClFitAndStore, for now, quit!....  " 
//                    << std::endl; exit(2); 
//       } 
         return true ;
      }  catch (...) { return false; }
    }
    void SSDRecDwnstrTracksAutre::dumpBeamTracksCmp(bool useKlmTrack) const {
//       if (fDebugIsOn) 
//        { std::cerr << " SSDRecDwnstrTracksAutre::dumpBeamTracksCmp, num Dwn tracks " << fTrs.size() << " And quit noe ! " << std::endl; exit(2); } 
   ...
   bool SSDRecDwnstrTracksAutre::doDwn3DKlmFitAndStore(int trID, const std::vector<myItStPt> &dataIn,   double pStartTmp) {
   
      bool isOKMult  = ((fInputSt2Pts.Size() > 0) && (fInputSt3Pts.Size() > 0) 
			    && (fInputSt5Pts.Size() > 0));   

       if (fDebugIsOn) {
         std::cerr << std::endl <<  " SSDRecDwnstrTracksAutre::doDwn3DKlmFitAndStore, spill " << fSubRunNum << " evt " 
...
//	 if (fDebugIsOn)  { std::cerr << " .... And quit now... " << std::endl; exit(2); } 
	 ...
	 return false;
       ...
       if (fFitterKlmFCN == nullptr) {
         std::cerr << " ..... SSDRecDwnstrTracksAutre::doDwn3DKlmFitAndStore, constructing a new instance of fFitterKlmFCN " << std::endl; 
...
//       if (fDebugIsOn) { std::cerr << " ...SSDRecDwnstrTracksAutre::doDwn3DKlmFitAndStore ... and quit for now... " << std::endl; exit(2); }
       return true;
       //
    } 
    void SSDRecDwnstrTracksAutre::dumpBeamTracksCmpKlm() const {
   ...
   void SSDRecDwnstrTracksAutre::transferKlmToClFit() {
     fTrs.clear();
...
//
   bool SSDRecDwnstrTracksAutre::HasGoodHighPForAlignment(double pMin, double chiSqMax) const {
