...
namespace emph {
  namespace ssdr {
    ...
    SSD3DTrackFitFCNAutre::SSD3DTrackFitFCNAutre(int runNum) :
    FCNBase(),
    fGeoService(),  
...
//
    ...
    SSD3DTrackFitFCNAutre::~SSD3DTrackFitFCNAutre() {
      if (fFOutResids.is_open()) fFOutResids.close();
    }  
    double SSD3DTrackFitFCNAutre::operator()(const std::vector<double> &pars) const {
    
      std::cerr << " Not Ready for Phase1c, quit here and now ... " << std::endl; exit(2);
      if (fDebugIsOn) std::cerr << "SSD3DTrackFitFCNAutre::operator, Start, number of track parameters " 
...
//         const double stepAlongZ = fIntegrationStep * (std::abs(pars[4])/120.);
	  ...
	  if (kSeT > 21) {
	    std::cerr << " SSD3DTrackFitFCNAutre::operator, Wrong residual indexing " << kSeT 
...
//	    std::cerr << " SSD3DTrackFitFCNAutre::operator(), problem with accessing Z coordinates, zUgly " 
...
//      if (fDebugIsOn) { std::cerr << " ......Chi Sq is " << chi2 << " And we keep going....  " << std::endl; }
     ...
     // 
    void SSD3DTrackFitFCNAutre::getZPos() const { 
     
      if (fDebugIsOn) std::cerr << " .... SSD3DTrackFitFCNAutre::getZPos(), fData.size() " << fData.size() << std::endl;   //
    ...
    void SSD3DTrackFitFCNAutre::OpenOutResids(const std::string &fNameStr) {
    ...
    void SSD3DTrackFitFCNAutre::SpitOutResids(int spill, int evtNum) {
      if (!fFOutResids.is_open()) return;
