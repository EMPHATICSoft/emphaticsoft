...
namespace emph {
  namespace ssdr {
    SSDRecUpstreamTgtAutre::SSDRecUpstreamTgtAutre() :
    ...
    size_t SSDRecUpstreamTgtAutre::recoXY(const art::Handle<std::vector<rb::SSDCluster> > aSSDClsPtr) {
      if (fEmVolAlP == nullptr) {
        std::cerr << " SSDRecUpstreamTgtAutre::recoXY, we require (for now, famous last word) the Volatile Alignment data. " << std::endl; 
...
//      const bool debugIsOn = ((fEvtNum == 1199) && (fSubRunNum == 10)); 
      ...
      if (debugIsOn) std::cerr << " SSDRecUpstreamTgtAutre::recoXY, spill " << fSubRunNum << " evt " << fEvtNum << std::endl;
      fTrXYs.clear();
...
//      if (debugIsOn) { std::cerr << " .... And quit enough for now, got one upstream track, enough! " << std::endl; exit(2); }
    ...
    void SSDRecUpstreamTgtAutre::dumpXYInforR(int uFlag) const {
      if (!fFOut3D.is_open()) {  
        std::ostringstream aNameStrStr;
        aNameStrStr << "./SSDRecUpstreamTgtAutre_" << fTokenJob << "_" << fRunNum << "_V1.txt"; 
