...
namespace rb{
  ...
  ADC::ADC(const emph::rawdata::WaveForm& wvfm)
  {
    _charge = this->CalcCharge(wvfm);
    _backovcharge = this->CalcBACkovCharge(wvfm);
    _time = 0;
  
  }

  //--------------------------------------------------
  //float ADC::Time(const emph::rawdata::WaveForm* wvfm, int adcOffset, int nhits, int start, int nsamp) const
    //returns time of hit according to steepest slope


  //--------------------------------------------------
  float ADC::CalcCharge(const emph::rawdata::WaveForm &wvfm, int adcOffset, int nhits, int start, int nsamp) const
  {
    int x1=start;
