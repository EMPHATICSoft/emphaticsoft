...
namespace emph {
	namespace arich_util {
		...
		PMT::PMT() :
			fPMTnum(-1), fName(""), fDarkRate(0), fQECorrection(1), fXTalk(0.01), fTriggerWin(20), fON(true)    //need to implement thes from GDML file
		{
		...
		PMT::PMT(int n, std::string name, double dark, double qecor, double fxtalk, std::vector<std::pair<double, double> > qe) :
			fPMTnum(n), fName(name), fDarkRate(dark), fQECorrection(qecor), fQE(qe), fXTalk(fxtalk)
		{
			rand_gen = new TRandom3(0);
		}
