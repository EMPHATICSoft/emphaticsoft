...
namespace emph {
  ...
  void SSDAlignment::Fit()
  {
	...
	for ( int i = 0; i < nstation; i++){
		...
		int nsensor = sta.NSSDs();
		for ( int j = 0; j < nsensor; j++){
			emph::geo::Detector sensor = sta.GetSSD(j);
			std::cout << "The " << j <<"-th SSD sensor in the " << i <<"-th SSD station is located at " << sensor.Pos()[0] << " " << sensor.Pos()[1] << " " << sensor.Pos()[2]+sta.Pos()[2] << " mm." << std::endl;
