...
class emph::MakeTrack : public art::EDProducer {
public:
  explicit MakeTrack(fhicl::ParameterSet const& pset);
  // The compiler-generated destructor is fine for non-base
  // classes without bare pointers or other resource use.

  // Plugins should not be copied or assigned.
  MakeTrack(MakeTrack const&) = delete;
  MakeTrack(MakeTrack&&) = delete;
  MakeTrack& operator=(MakeTrack const&) = delete;
  MakeTrack& operator=(MakeTrack&&) = delete;
...
//--------------------------------------------------
void emph::MakeTrack::beginRun(art::Run&)
{
...
//--------------------------------------------------
bool emph::MakeTrack::CompareByRow(const art::Ptr<emph::rawdata::SSDRawDigit>& a,
					 const art::Ptr<emph::rawdata::SSDRawDigit>& b)
{
  return a->Row() < b->Row();
}
...
//--------------------------------------------------
void emph::MakeTrack::SortByRow(art::PtrVector<emph::rawdata::SSDRawDigit>& dl)
{
  std::stable_sort(dl.begin(), dl.end(), CompareByRow);
}

//--------------------------------------------------
void emph::MakeTrack::FormClusters(art::PtrVector<emph::rawdata::SSDRawDigit> sensDigits,
		  std::vector<rb::SSDCluster>* sensClusters,
		  int station, int sensor)
{ 
...
void emph::MakeTrack::produce(art::Event& evt)
{
  ...
  if (fCheckDQ){
    ...
    if(!eventqual){
      mf::LogError("MakeTrack")<<"No Data Quality product found in event but CheckDQ set to true!";
      abort();
    }
