...
//  }
  ...
  void emph::CaloProd::beginJob()
  {
    ...
    for (int i=0; i<nchancal; ++i){
          fLGCaloIntCharge[i] = tfs->make<TH1F>(Form("LGCaloIntCharge_%d",i),Form("LG Calo Integrated Charge %d",i),1000,-500.,7500.); //200,0,26000
    }
  ...
  void emph::CaloProd::fWantLGCalo(art::Handle< std::vector<rawdata::WaveForm> > & wvfmH)
    {
	 ...
	 std::cout<<"This run has bad calorimeter waveforms :/"<<std::endl;
      ...
      //if (fEvtNum != 703) fMakeWaveFormPlots = false; //r2463 --> neg int charge on block 4 (center) --> double peak
      //if (fEvtNum != 8266) fMakeWaveFormPlots = false; //r2463 neg int charge block 2 --> waveform in unexpected spot (very close to the end of the window)
      //if (fEvtNum != 16797) fMakeWaveFormPlots = false;
      //if (fEvtNum != 7120) fMakeWaveFormPlots = false; //double peak + too close
      //if (fEvtNum != 10358) fMakeWaveFormPlots = false; //same thing
      //if (fEvtNum != 13508) fMakeWaveFormPlots = false; //too close
      //if (fEvtNum != 16280) fMakeWaveFormPlots = false; //possible double peak + too close
...
//std::cout<<"hey"<<std::endl;
		...
		getBaseline(adcvals,8.0,10,true);
		   ...
		   if (intchg > 0){ // || fEvtNum == 8266){
		   //float intchg = getIntChargePlots(adcvals,detchan);
		   //negative intchg indicates something weird in the waveform
		   //i.e. it's not in a place you'd expect, cutoff by the 108-bin window,
		   //messing up the baseline calculation
		   //we only include info from good waveforms in avg wfm plots + intchg plots
		   ...
		   //messing up the baseline calculation
		   //if (intchg < 0) 
		   fLGCaloIntCharge[detchan]->Fill(intchg); 
		   ...
		   calohits.push_back(onehit);
              ...
              //also write channel
              //std::cout<<"CALO HIT: "<<fEvtNum<<std::endl;
	      }
  ...
  float emph::CaloProd::getIntChargePlots(const std::vector<uint16_t> &adcvals, int detchan){
    ...
    if (sum < 0){
       std::cout<<"Negative sum found (Block "<<detchan<<"): "<<fEvtNum<<std::endl;
  ...
  void emph::CaloProd::produce(art::Event& evt)
  {
	...
	std::string fG4Label = "geantgen";
        art::Handle< std::vector<sim::TOPAZLGHit> > TOPAZLGHitH;
        try {
           evt.getByLabel(fG4Label,TOPAZLGHitH);
        }
        catch(...) {
        std::cout << "WARNING: No TOPAZLGHits found!" << std::endl;
        }
	if (!TOPAZLGHitH->empty()) {
           for (size_t idx=0; idx < TOPAZLGHitH->size(); ++idx) {
	       const sim::TOPAZLGHit& topazlghit = (*TOPAZLGHitH)[idx];
	       //std::cout<<"Block Number: "<<topazlghit.GetBlockNumber()<<std::endl;
	       //i think 11 is the center
	       std::cout<<"PID: "<<topazlghit.GetPId()<<std::endl;
	       //if (topazlghit.GetBlockNumber() == 11)
	   }  
	}
    }
