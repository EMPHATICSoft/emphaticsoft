...
namespace arichreco{
   ...
   double Aerogel::getThickness() {return thickness;}

   double Aerogel::getRefractiveIndex() {return refracIndex;}

   double Aerogel::getZPos() {return zPos;}

   //Note refractive index of upstream material
   void Aerogel::setUpIndex(double n) { this->upIndex = n;}

   //Note refractive index of downstream material
   void Aerogel::setDownIndex(double n) {this->downIndex = n;}

   //Calculate Cherenkov angle
   double Aerogel::calcChAngle(double beta) {return acos(1.0 / (refracIndex * beta));}
   ...
   */
    ...
    */
    return (TMath::Abs(pos[0]) <= width/2) && (TMath::Abs(pos[1]) <= height/2.)
                  && ((pos[2] - zPos) <= thickness) && ((pos[2] - zPos) >= 0);
   }
   ...
   return abs((zPos + thickness - pa->pos[2]) / pa->dir[2]);
    ...
    void Aerogel::exitAerogel(arichreco::Photon* ph, bool refract, double n2) {
      ...
      */
      ...
      double zDist = 0;
      if (forwardZ) {zDist = (zPos + thickness - ph->pos[2]) / ph->dir[2];}
      else { zDist = (zPos - ph->pos[2]) / ph->dir[2];}
