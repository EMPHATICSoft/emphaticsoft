...
//                    WARNING
// We are assuming:
// 1) The file names must be like this: Run1_ . That is they start with Run, then there is the run number and then an underscore (int FilesSplitter::makeFilesList(string runNumber,std::string dir))
// 2) The 3 stations must be named dut, downstream and upstream                                                                  (int FilesSplitter::makeFilesList(string runNumber,std::string dir))
// 3) The files are contained in 3 subdirectories called CC_DUT/Burst Archive, CC_DS/Burst Archive, CC_US/Burst Archive          (int FilesSplitter::makeFilesList(string runNumber,std::string dir))
// 4)The station numbers are dut => station = 4, upstream => station = 2 and downstream => station = 0                           (int FilesSplitter::merge(void))
//
///////////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char **argv)
{
  ...
  //////
  FilesSplitter theFilesSplitter;
  ...
  theFilesSplitter.setStationNames(stationNames);

  for (int runNumber = firstRunNumber; runNumber <= lastRunNumber; runNumber++)
  {
    cout << __PRETTY_FUNCTION__ << "\tMerging run " << runNumber << endl;
    if (theFilesSplitter.makeFilesList(runNumber, inputFilesDir) == -1)
      continue;

    theFilesSplitter.makeStationBuffers();
    theFilesSplitter.merge();
    theFilesSplitter.writeMergedFiles(outputFilesDir, "");
  }
