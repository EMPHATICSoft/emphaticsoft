...
//-----------------------------------------------------------------------
int FilesSplitter::makeFilesList(unsigned int runNumber, std::string dir)
{
...
//-----------------------------------------------------------------------
int FilesSplitter::makeFilesList(string runNumber, std::string dir)
{
	...
	for (unsigned int station = 0; station < stationNames_.size(); station++)
	{
		...
		else if (stationNames_[station].find("FER") != string::npos)
		{
			subDir = dir + "/";
		}
		else if (stationNames_[station].find("fed") != string::npos)
		{
			subDir = dir + "CMSOuterTracker/Data";
		}
		else if (stationNames_[station].find("nim") != string::npos)
		{
			subDir = dir + "NimPlus/";
		}
		else if (stationNames_[station].find("RD53Middleware") != string::npos)
		{
			std::cout << __PRETTY_FUNCTION__ << "RD53" << std::endl;
			subDir = dir + "RD53Middleware/";
		}
		...
		while ((pDirEntries = readdir(pDir)))
		{
			...
			std::string cmsRunNumber;
			if (atoi(runNumber.c_str()) < 10)
				cmsRunNumber = "00000" + runNumber;
			else if (atoi(runNumber.c_str()) < 100)
				cmsRunNumber = "0000" + runNumber;
			else if (atoi(runNumber.c_str()) < 1000)
				cmsRunNumber = "000" + runNumber;
			else if (atoi(runNumber.c_str()) < 10000)
				cmsRunNumber = "00" + runNumber;
			else if (atoi(runNumber.c_str()) < 100000)
				cmsRunNumber = "0" + runNumber;
			else
				cmsRunNumber = runNumber;
			//cout << __PRETTY_FUNCTION__ << fileName << "=" << string("run_" + cmsRunNumber + "_") << " found? " << fileName.find("run_" + cmsRunNumber + "_") << " station name:" << stationNames_[station] << endl;
			if ((fileName.find("Run" + runNumber + "_") != string::npos && fileName.find(stationNames_[station]) != string::npos) || fileName.find("Run" + cmsRunNumber + "_") != string::npos)
			{

				//std::cout << "Run Number " << runNumber << std::endl;
				cout << __PRETTY_FUNCTION__ << fileName << " station name: " << stationNames_[station] << endl;
				std::cout << __PRETTY_FUNCTION__ << std::string("Run" + cmsRunNumber + "_") << std::endl;
				tmpFilesList.insert(pair<string, string>(stationNames_[station], subDir + "/" + fileName));
				if (fileName.find("Run" + cmsRunNumber + "_") != string::npos)
					rd53FileName_ = subDir + "/" + fileName;
			}
		}
...
//-----------------------------------------------------------------------
int FilesSplitter::makeStationBuffers(void)
{
	if (filesList_.size() == 0)
	{
	...
	for (unsigned int s = 0; s < 4; s++)
		for (std::map<uint64_t, PxEvent *>::iterator it = memory_[s].begin(); it != memory_[s].end(); it++)
		{
			if (it->second->getTriggerNumber() != -1) //  &&  TriggerToRemove.find(it->second->getTriggerNumber())!=TriggerToRemove.end())
			{
				...
				if (currentEvent->getCurrentBCONumber() == -1)
...
//-----------------------------------------------------------------------
int FilesSplitter::writeMergedFiles(string filesDirectory, string fileName)
{
	...
	//std::cout << "Run Number " << runNumber << std::endl;
	const int bco2s = 36*4;
	int subrunNumber = 1;
	string subrunNumber_ = to_string(subrunNumber);
	...
	for (map<unsigned int, EventMonicelli>::iterator eventsIt = theRun.getEvents().begin(); eventsIt != theRun.getEvents().end(); eventsIt++)
	{
		...
		if((bco-prevbco)*bco2s*1e-9>40 && prevbco > 0){
			outFile.close();
			subrunNumber++;
			subrunNumber_ = to_string(subrunNumber);
			fileName = "Run" + runNumber_ + "_" + subrunNumber_ + ".dat";
			cout << "[" << __LINE__ << "]" << __PRETTY_FUNCTION__ << " Saving events in file: " << filesDirectory + fileName << endl;

			outFile.open((filesDirectory + fileName).c_str(), ios::binary);
		}
		prevbco = bco;
	}
...
//-----------------------------------------------------------------------
int FilesSplitter::readMergedFiles(string fileName)
{
...
//-----------------------------------------------------------------------
int FilesSplitter::readStationBuffers(void)
{
	//Reading the file
...
//-----------------------------------------------------------------------
int FilesSplitter::readFile(void)
{
	//Reading the file
	// 1 Byte (unsigend char) = Number of 8 bytes words in the following block (max 183)
	// 8 Bytes: the first word in a block is the time stamp in computer seconds
...
//-----------------------------------------------------------------------
std::string FilesSplitter::BitString(const std::vector<uint32_t> &cData, uint32_t pOffset, uint32_t pWidth)
{
...
uint64_t FilesSplitter::mortonEncode(uint32_t pEvenWord, uint32_t pOddWord)
{
...
void FilesSplitter::printWord(uint32_t w)
{
...
void FilesSplitter::printWord(unsigned long long w)
{
...
uint32_t FilesSplitter::reverseWord(uint32_t w)
{
