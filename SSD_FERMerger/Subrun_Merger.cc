...
int main(int argc, char **argv)
{
  ...
  int runNumber, subRunNumber;

  if (argc < 2 || argc > 3)
  {
    cout << __PRETTY_FUNCTION__ << "\tUsage: ./Merger runNumber or ./Merger runNumber subRunNumber" << endl;
    exit(EXIT_FAILURE);
  }
  else if (argc == 2)
  {
  ...
  int startSub = 1, endSub = MaxSubRun;
  if(subRunNumber > 0){
    startSub = subRunNumber;
    endSub = subRunNumber;
  }
  for (int subrunNumber = startSub; subrunNumber <= endSub; subrunNumber++){
