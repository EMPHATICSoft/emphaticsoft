...
double Lhist1::GetXrms() {
    ...
    double mean = GetXave();
    for (Lbins::bin B=1; B<=GetXNbins(); ++B) {
        retval += GetBinBin(B).GetValue() * SQR(Xbins.Center(B) -mean);
    }
    retval /= GetXsum();
    return sqrt(retval);
}
...
double Lhist1::MinVal() const {
    Lbins::bin minbin = MinBin();
    return (*HistArray)(minbin).GetValue();
}
double Lhist1::MaxVal() const {
    Lbins::bin maxbin = MaxBin();
    return (*HistArray)(maxbin).GetValue();
}
...
Lbins::bin Lhist1::MaxBin() const {
    ...
    for (Lbins::bin Bx=1; Bx<=GetXNbins(); ++Bx) {
        Lhistbin thisbin =(*HistArray)(Bx);
        if (!thisbin.virgin && thisbin.GetValue() > maxval) {
            retval = Bx;
            maxval = thisbin.GetValue();
        }
    }
    return retval;
}
