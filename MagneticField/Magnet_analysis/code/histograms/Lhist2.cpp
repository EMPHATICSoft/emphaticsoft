    ...
    for (Lbins::bin i=Lbins::U; i<=GetXNbins(); ++i) {
        for (Lbins::bin j=Lbins::U; j<=GetYNbins(); ++j) {
            if ((*rhs.HistArray)(i,j).GetValue() == 0.0) {
                // Don't flag divide-by-zero for over/underflow bins
...
Lhist2 Lhist2::GetErrorHisto() const {
    Lhist2 retval(*this);
    for (Lbins::bin i=Lbins::U; i<=GetXNbins(); ++i) {
        for (Lbins::bin j=Lbins::U; j<=GetYNbins(); ++j) {
            (*retval.HistArray)(i,j).virgin = (*HistArray)(i,j).virgin;
            if ((*retval.HistArray)(i,j).virgin) {
                (*retval.HistArray)(i,j).SetValue(0);
            } else {
                (*retval.HistArray)(i,j).SetValue( (*HistArray)(i,j).GetError() );
            }
            (*retval.HistArray)(i,j).SetError(0);
        }
    }
    return retval;
}
...
double Lhist2::GetXrms() {
    ...
    for (Lbins::bin B=1; B<=GetXNbins(); ++B) {
        for (Lbins::bin By=1; By<=GetYNbins(); ++By) {
            retval += GetBinBin(B,By).GetValue() * SQR(Xbins.Center(B) -mean);
        }
    }
    retval /= GetXYsum();
    return sqrt(retval);
...
double Lhist2::GetYrms() {
    ...
    for (Lbins::bin B=1; B<=GetYNbins(); ++B) {
        for (Lbins::bin Bx=1; Bx<=GetXNbins(); ++Bx) {
            retval += GetBinBin(Bx,B).GetValue() * SQR(Ybins.Center(B) -mean);
        }
    }
    retval /= GetXYsum();
    return sqrt(retval);
...
double Lhist2::MinVal() const {
    ...
    minbins = MinBin();
    return (*HistArray)(minbins(1),minbins(2)).GetValue();
}
double Lhist2::MaxVal() const {
    ra<Lbins::bin> maxbins(2);
    maxbins = MaxBin();
    return (*HistArray)(maxbins(1),maxbins(2)).GetValue();
}
ra<Lbins::bin> Lhist2::MinBin() const {
    ra<Lbins::bin> retval(2);      retval = Lbins::U;
    double minval = +Dbig;
    for (Lbins::bin Bx=1; Bx<=GetXNbins(); ++Bx) {
        for (Lbins::bin By=1; By<=GetYNbins(); ++By) {
            Lhistbin thisbin =(*HistArray)(Bx,By);
            if (!thisbin.virgin && thisbin.GetValue() < minval) {
                retval(1) = Bx;     retval(2) = By;
                minval = thisbin.GetValue();
            }
        }
    }
    return retval;
...
ra<Lbins::bin> Lhist2::MaxBin() const {
    ra<int> retval(2);
    double minval = -Dbig;      retval = Lbins::U;
    for (Lbins::bin Bx=1; Bx<=GetXNbins(); ++Bx) {
        for (Lbins::bin By=1; By<=GetYNbins(); ++By) {
            Lhistbin thisbin =(*HistArray)(Bx,By);
            if (!thisbin.virgin && thisbin.GetValue() > minval) {
