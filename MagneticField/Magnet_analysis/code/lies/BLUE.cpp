...
double BLUE::PDGscale() {
    for (int i=1; i<=Nexp; ++i) {
        for (int j=i+1; j<=Nexp; ++j) {
            if ( (*covariances)(i,j) != 0) {
                LppUrk.LppIssue(214, "BLUE::PDGscale()");
                break;
            }
        }
    }
    // Are any measurements disallowed for having too large an uncertainty?
    int Nuse = 0;
    ra<bool> useme(Nexp);
    ...
    if (Nuse <= 1) {
        LppUrk.LppIssue(215, "BLUE::PDGscale()");
        return 1.0;
    }
    ...
    return max(1.0, sqrt(used_only.getChi2()/(Nuse-1)) );
}
