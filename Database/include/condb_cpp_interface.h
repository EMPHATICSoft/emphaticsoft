...
class ConditionsDB : private ConditionsDBBase {
 ...
 ConditionsDB(const std::string& base_url) :
  ...
  template <size_t NUM_COL>
    ConditionsDBResponse<NUM_COL> query(const std::string& folder, const std::array<std::string, NUM_COL>& column_names, 
    ...
    std::string data_resrc_id = "/get?table=" + folder + "&t=" + std::to_string(instant) + "&columns=";
    bool isFirst = true;
