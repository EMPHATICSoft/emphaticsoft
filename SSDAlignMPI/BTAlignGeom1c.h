...
///           Upgrade for Phase1c yeah.. Cloning is bad.. but... 
/// \author  lebrun@fnal.gov
/// \date
////////////////////////////////////////////////////////////////////////
#ifndef BTALIGNGEOM1C_H
#define BTALIGNGEOM1C_H
...
namespace emph{ 
  namespace rbal {
    ...
    class BTAlignGeom1c {
      private: 
        BTAlignGeom1c();
        const size_t fNumStations = 7; // For Phase1b 
        const size_t fNumSensorsXorY = 9; // Station 4 and 5 have 2 sensors, so, 4*1 + 2*2 
	...
	const size_t fNumStrips = 640; // a bit more correct.. 
	//
	...
	static BTAlignGeom1c* instancePtr;
	
      public:
      
        inline static BTAlignGeom1c* getInstance() {
	 if ( instancePtr == NULL ) instancePtr = new BTAlignGeom1c();
	 return instancePtr;
	}
      
        BTAlignGeom1c(const BTAlignGeom1c&) = delete;   // no copy constructor. 
...
//             std::cerr << "  BTAlignGeom1c::ZPos, View X, kSe, " << kSe << " Z " << fZPosX[kSe] << std::endl;
	    ...
	    default : { 
	      std::cerr << " BTAlignGeom1c::ZPos, unknown view " << view << " fatal, quit " << std::endl; 
	      exit(2);  } 
	  }
...
//	     if (sensor >= fZNomPosX.size()) { std::cerr .... No checks!. 
	    ...
	    default : { 
	      std::cerr << " BTAlignGeom1c::TrPos, unknown view " << view << " fatal, quit " << std::endl; 
	      exit(2);  } 
	   }
	  return 0.;  // Should never happen.. 
	}
...
//	     if (sensor >= fZNomPosX.size()) { std::cerr .... No checks!. 
	    ...
	    default : { 
	      std::cerr << " BTAlignGeom1c::TrNomPos, unknown view " << view << " fatal, quit " << std::endl; 
	      exit(2);  } 
	   }
	  return 0.;  // Should never happen.. 
	}
...
//	     if (sensor >= fZNomPosX.size()) { std::cerr .... No checks!. 
	    ...
	    default : { 
	      std::cerr << " BTAlignGeom1c::DeltaPitch, unknown view " << view << " fatal, quit " << std::endl; 
	      exit(2);  } 
	   }
	  return 0.;  // Should never happen.. 
	}
...
//	     if (sensor >= fZNomPosX.size()) { std::cerr .... No checks!. 
	    ...
	    default : { 
	      std::cerr << " BTAlignGeom1c::Roll, unknown view " << view << " fatal, quit " << std::endl; 
	      exit(2);  } 
	   }
	  return 0.;  // Should never happen..
	}
...
//	     if (sensor >= fZNomPosX.size()) { std::cerr .... No checks!. 
	    ...
	    default : { 
	      std::cerr << " BTAlignGeom1c::RollCenter, unknown view " << view << " fatal, quit " << std::endl; 
	      exit(2);  } 
	   }
	  return 0.;  // Should never happen..
	}
...
//	     if (sensor >= fZNomPosX.size()) { std::cerr .... No checks!. 
	    ...
	    default : { 
	      std::cerr << " BTAlignGeom1c::UnknownUncert, unknown view " << view << " fatal, quit " << std::endl; 
	      exit(2);  } 
	   }
	  return 0.;  // Should never happen..
	}
...
//	     if (sensor >= fZNomPosX.size()) { std::cerr .... No checks!. 
	    ...
	    default : { 
	      std::cerr << " BTAlignGeom1c::MultScatUncert, unknown view " << view << " fatal, quit " << std::endl; 
	      exit(2);  } 
	   }
	  return 0.;  // Should never happen..
	}
